...
let DirectMessages = class DirectMessages{
    ...
    static getChannels(userId,page) {
        return new Promise(async(resolve, reject) => {
            ...
            db.query('SELECT * from dm_channels WHERE dm_user1 = ? OR dm_user2 = ? LIMIT 20 OFFSET ? ',[userId, userId,skip], (err, result) => {
                if(err) reject(new Error(err))
                else resolve(result)
            })
        })
    ...
    static getChannel(channelId,userId,page) {
        return new Promise(async(resolve, reject) => {
            ...
            db.query('SELECT * from dm_post LEFT JOIN members ON dm_post.dm_post_author = members.member_id WHERE dm_post_dmId = ? LIMIT 10 OFFSET ?',[channelId,skip], (err, result) => {
                if(err) reject(new Error(err))
                else {
    ...
    static postChannel(message,title,author,client) {
        return new Promise(async(resolve, reject) => {
            ...
            const time = Date.now()
            db.query('INSERT INTO `dm_channels`(`dm_id2`, `dm_user1`, `dm_user2`, `dm_title`) VALUES (?,?,(SELECT members.member_id FROM members WHERE members.member_pseudo = ?),?)',[id2,author,client,title],(err,result) => {
                if(err) {
        ...
        return new Promise(async(resolve, reject) => {
            db.query('SELECT * from dm_channels WHERE dm_id = ?',[channelId], async(err, result) => {
                if(err) reject(new Error(err))
                    ...
                    const date = Date.now()
                    db.query('INSERT INTO dm_post (`dm_post_author`, `dm_post_client`, `dm_post_dmId`, `dm_post_message`, `dm_post_time`, `dm_post_lu`) VALUES (?,?,?,?,?,?)',[authorP,clientP,channelIdP,message,date,0], (err, result) => {
                        if(err) reject(new Error(err))
                        else resolve(true)
                    })
                }
